...
const SOCKET_RELATIVE_PATH: &str = ".config/cascade/fileprovider.sock";

/// Resolve the default File Provider bridge socket path.
pub fn default_socket_path() -> Result<PathBuf> {
    let home =
        std::env::var_os("HOME").context("HOME is required to locate the File Provider socket")?;
    Ok(PathBuf::from(home).join(SOCKET_RELATIVE_PATH))
}

/// Client for the Swift File Provider extension bridge.
#[derive(Debug)]
...
impl FileProviderBridge {
        ...
        T: DeserializeOwned,
    {
        ...
        if let Some(error) = response.error {
            bail!(
                "File Provider bridge request {} failed: {error}",
                request.method
            );
        }
        ...
        let result = response.result.with_context(|| {
            format!(
                "File Provider bridge request {} returned no result",
                request.method
            )
        })?;
        serde_json::from_value(result).with_context(|| {
            format!(
                "invalid File Provider bridge response for {}",
                request.method
            )
        })
    }
    ...
    async fn send_request(&self, request: &Request) -> Result<Response> {
        let mut stream = UnixStream::connect(&self.socket_path)
            ...
            .with_context(|| {
                format!(
                    "connect to File Provider socket {}",
        ...
        stream
            .write_all(&encoded)
            .await
            .context("write File Provider request")?;

        let mut len = [0u8; 4];
        stream
            .read_exact(&mut len)
            .await
            .context("read File Provider response length")?;
        let body_len = u32::from_be_bytes(len) as usize;
        let mut body = vec![0u8; body_len];
        stream
            .read_exact(&mut body)
            .await
            .context("read File Provider response body")?;
        serde_json::from_slice(&body).context("decode File Provider response")
    }
...
mod tests {
    ...
    fn build_request_increments_request_ids() {
        let bridge = FileProviderBridge::new("/tmp/cascade-fileprovider-test.sock");
    ...
    async fn request_round_trips_over_unix_socket() {
        ...
        let socket_path = std::env::temp_dir().join(format!(
            "cascade-fileprovider-{}-roundtrip.sock",
            std::process::id()
        ));
