...
use cascade_presenter_fskit::bridge::FSKitBridge;
use cascade_presenter_fskit::items::FSKitItem;
...
fn bridge_builds_valid_protocol_request() {
    let bridge = FSKitBridge::new("/tmp/cascade-test.sock");
...
async fn bridge_round_trips_over_unix_socket() {
    ...
    let socket_path = std::env::temp_dir().join(format!(
        "cascade-fskit-integration-{}-roundtrip.sock",
        std::process::id()
    ));
...
#[test]
fn vfs_item_to_fskit_item_preserves_fields() {
    let item = sample_vfs_item();
    let fskit_item = FSKitItem::from(item);

    assert_eq!(fskit_item.id, "gdrive:file123");
    assert_eq!(fskit_item.parent_id, "gdrive:root");
    assert_eq!(fskit_item.filename, "report.pdf");
    assert!(!fskit_item.is_directory);
    assert_eq!(fskit_item.size, Some(8192));
    assert_eq!(fskit_item.content_type, Some("application/pdf".to_string()));
    assert_eq!(fskit_item.cache_state, CacheState::Cached);
    assert!(fskit_item.last_modified.is_some());
}

#[test]
fn fskit_item_round_trips_to_vfs_item() {
    let original = sample_vfs_item();
    let fskit_item = FSKitItem::from(original.clone());
    let round_tripped = VfsItem::try_from(fskit_item).unwrap();
...
#[test]
fn fskit_item_rejects_invalid_date() {
    let mut fskit_item = FSKitItem::from(sample_vfs_item());
    fskit_item.last_modified = Some("not-a-valid-date".to_string());

    let result = VfsItem::try_from(fskit_item);
    assert!(result.is_err());
    assert!(
        result
            .unwrap_err()
            .to_string()
            .contains("invalid FSKit modification date"),
    );
}

// ---------------------------------------------------------------------------
// Lookup simulation via protocol
// ---------------------------------------------------------------------------

#[test]
fn lookup_request_builds_correct_protocol_message() {
    let bridge = FSKitBridge::new("/tmp/cascade-test.sock");
    ...
    assert_eq!(request.method, "lookupItem");
    assert_eq!(
        request.params,
        json!({"parent_id": "gdrive:root", "name": "report.pdf"}),
    );
}
...
async fn presenter_operations_fail_outside_macos() {
    ...
    use cascade_presenter_fskit::FSKitPresenter;
    use std::path::Path;

    let presenter = FSKitPresenter::new("/tmp/cascade-test.sock");
...
mod proptests {
    ...
    proptest! {
        ...
        fn vfs_item_round_trip_preserves_fields(
            ...
            let fskit_item = FSKitItem::from(item.clone());
            let round_tripped = VfsItem::try_from(fskit_item).unwrap();
