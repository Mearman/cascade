...
/// FSKit-facing representation of a VFS item.
///
/// This struct is the JSON shape sent over the bridge to the Swift `FSKit`
/// extension. The extension deserialises it to construct `FSItem` attributes
/// for the kernel.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct FSKitItem {
    pub id: String,
...
impl From<VfsItem> for FSKitItem {
...
impl TryFrom<FSKitItem> for VfsItem {
    type Error = anyhow::Error;

    fn try_from(item: FSKitItem) -> Result<Self> {
        let mod_time = item
            .last_modified
            .map(|value| {
                chrono::DateTime::parse_from_rfc3339(&value)
                    .with_context(|| format!("invalid FSKit modification date: {value}"))
...
mod tests {
    ...
    #[test]
    fn maps_vfs_item_to_fskit_item() {
        let item = FSKitItem::from(vfs_item());
    ...
    #[test]
    fn maps_fskit_item_to_vfs_item() {
        let fskit_item = FSKitItem::from(vfs_item());
        let item = VfsItem::try_from(fskit_item).unwrap();
    ...
    #[test]
    fn rejects_invalid_fskit_date() {
        let mut item = FSKitItem::from(vfs_item());
        item.last_modified = Some("not-a-date".to_string());

        let err = VfsItem::try_from(item).unwrap_err().to_string();
        assert!(err.contains("invalid FSKit modification date"));
    }
}
