...
The HTTP client is built fresh for every request with connection pooling disabled (`pool_max_idle_per_host(0)`) and HTTP/1.1 forced (`http1_only`), in both `DriveClient::http` and the token-refresh path. Both build their client through the shared `build_unpooled_http1_client` helper, which returns the builder error rather than falling back to a default client — a default client re-enables pooling, the configuration the workaround forbids. The load-bearing knob is `pool_max_idle_per_host(0)`: it stops a stale idle connection ever being reused. The `http1_only` call is in practice redundant — the workspace `reqwest` is built with `default-features = false` and no `http2` feature (and has been since the first commit), so the client cannot negotiate HTTP/2 regardless; it is kept only as a guard should that feature ever be enabled. This is a deliberate workaround, not an oversight, and it must not be reverted without a confirmed root cause and a passing reproduction.

The hang it works around was only ever observed through the WebDAV presenter, where an axum/hyper-1.x server and a reqwest/hyper-1.x client share one tokio runtime. A backend TLS handshake opened while the server was mid-response never completed, so the second use of a pooled connection wedged the task. The WebDAV presenter independently carries the matching half of the workaround — it forces `Connection: close` on every response and runs the backend write on an isolated thread with its own runtime.
...
A later faithful-topology reproduction (`crates/presenter-webdav/tests/tls_topology_repro.rs`) recreated the nested axum-handler-over-shared-runtime topology and bisected pooling, runtime isolation, `Connection: close`, worker count, concurrency, and idle-pooled-connection drop-and-reuse — against both a synthetic local TLS server and the real, no-auth `www.googleapis.com` discovery frontend. None reproduced the hang. The trigger is therefore narrowed to the authenticated Drive upload host; the only remaining path to a confirmed root cause is the human-in-the-loop capture documented in [`docs/tls-deadlock-capture.md`](tls-deadlock-capture.md). The workaround stays until that capture confirms the cause and a reproduction passes.

An upstream survey settles whether a dependency bump could retire the workaround: it cannot. The workspace is already on the latest hyper (1.10.1) and hyper-util (0.1.20) — there is nothing newer to move to — and the workaround was written against exactly these versions. reqwest 0.13's changes are HTTP/3-only and irrelevant to this HTTP/1.1 path, and the build is already past 0.12.8's pooled-connection-expiry fix. The matching upstream report, [hyper#3611](https://github.com/hyperium/hyper/issues/3611) ("server hangs handling a request from a client on the same host"), was closed *not planned*: the maintainer's diagnosis is that the hyper client connection driver must be spawned and polled concurrently, never stranded — "when the client connection future completes, that means the connection is closed". The class of bug is a stranded pooled-connection driver: a connection whose driver task was spawned on a runtime (or task context) that later stops polling it wedges on its next reuse. `pool_max_idle_per_host(0)` is the idiomatic mitigation for that — a fresh connection, hence a freshly-polled driver, every request — which is exactly what this code does, at the cost of a TLS handshake per request. Reclaiming that cost is therefore an architectural change, not a version bump: a single, long-lived, *pooled* `reqwest::Client` owned by the daemon's stable main runtime (dropping both the per-request client and the WebDAV presenter's `run_isolated_blocking`), so every connection driver lives and is polled on one runtime for the daemon's lifetime. The faithful-topology repro suggests that is now safe, but it must still be confirmed by the authenticated-Drive capture above before the workaround is removed.

That target architecture is now the **default** (`DiagHttpMode::PooledShared`). The daemon's start path builds one long-lived pooled `reqwest::Client` on the main runtime, wraps it in the existing `cascade_engine::portable::native::ReqwestClient` adapter, and injects it as `Arc<dyn HttpClient>` through `create_backend_with_store_and_http` into the native `DriveClient` — bringing native onto the same injected-client contract the `portable` feature already uses. Because a pooled connection driver must stay on the stable runtime and never inside `run_isolated_blocking`'s transient one, the same mode is read once (one `OnceLock`, so the two halves can never disagree) and also threaded to the WebDAV presenter as `skip_isolation`: by default the PUT handler awaits the backend write directly on the axum runtime, so the shared driver stays polled where it was spawned.

The former workaround is preserved as an **escape hatch**: `CASCADE_GDRIVE_HTTP_DIAG=unpooled-legacy` reverts to the per-request unpooled HTTP/1.1 client with `run_isolated_blocking` and the isolated path, instantly and without a rebuild, should the shared client ever wedge in production. `Connection: close` is left forced in both paths (cheap, defensive). The `parse_diag_mode` policy — default → `PooledShared`, `unpooled-legacy`/`unpooled-http1` → the workaround — is unit-tested (`diag_mode_default_is_pooled_shared`).

The default was flipped after two real-endpoint captures passed (2026-06-07, full results in [`docs/tls-deadlock-capture.md`](tls-deadlock-capture.md)): **68 sequential WebDAV uploads** to a real authenticated Drive account in `pooled-shared` mode — a 20-upload back-to-back burst, files to 512 KB, idle gaps to 50 s — all completed with the `before-send`/`after-headers` ledger balanced and no wedge, plus a confirmation run that the daemon now uses the shared client with `skip_isolation` by default. The residual collapse — deleting `build_unpooled_http1_client` / the per-request path / `run_isolated_blocking` / the `DiagHttpMode` selector entirely — is deferred until the new default has soaked in real use; the escape hatch stays until then. The original hang was never reproduced in any configuration, so the captures prove the fix works in those runs but not harness sensitivity; the escape hatch is the safeguard for that residual uncertainty.
