...
That target architecture is now implemented as an **opt-in migration seam**, with the production default unchanged. A `pooled-shared` value for `CASCADE_GDRIVE_HTTP_DIAG` (a new `DiagHttpMode::PooledShared`) tells the daemon's start path to build one long-lived pooled `reqwest::Client` on the main runtime, wrap it in the existing `cascade_engine::portable::native::ReqwestClient` adapter, and inject it as `Arc<dyn HttpClient>` through `create_backend_with_store_and_http` into the native `DriveClient` — bringing native onto the same injected-client contract the `portable` feature already uses. Because a pooled connection driver must stay on the stable runtime and never inside `run_isolated_blocking`'s transient one, the same mode is read once and also threaded to the WebDAV presenter as `skip_isolation`: in `pooled-shared` the PUT handler awaits the backend write directly on the axum runtime; in every other mode it keeps `block_in_place` + `run_isolated_blocking` exactly as before. The two halves consult one `OnceLock`, so they cannot disagree. With the variable unset (or any other value) the daemon injects no client, the backend builds its per-request unpooled HTTP/1.1 client as today, isolation stays on, and `Connection: close` stays forced — byte-for-byte the workaround (guarded by `default_drive_client_has_no_injected_http_client` and `default_diag_mode_is_not_pooled_shared`).
