...
### Google Drive HTTP client (resolved TLS deadlock)

Every Drive API call and `OAuth2` refresh routes through a single injected `Arc<dyn HttpClient>`. The daemon builds one long-lived, pooled `reqwest::Client` on its stable main runtime, wraps it in `cascade_engine::portable::native::ReqwestClient`, and injects it through `create_backend_with_store_and_http` into the `DriveClient` — the same injected-client contract the `portable` feature uses, so native and portable share one code path. The WebDAV PUT handler awaits the backend write directly on the axum runtime. There is no per-request client, no connection-pooling toggle, and no isolated runtime: the pooled connection driver lives and is polled on one runtime for the daemon's lifetime, which is what keeps it from being stranded.

This replaced an earlier workaround, kept here because it explains why the architecture is shaped this way. The original hang was only ever observed through the WebDAV presenter, where an axum/hyper-1.x server and a reqwest/hyper-1.x client share one tokio runtime. The presenter used to run each backend write inside `run_isolated_blocking` — a transient current-thread runtime on a separate OS thread — and the backend built a fresh, unpooled, HTTP/1.1-only `reqwest::Client` per request (`pool_max_idle_per_host(0)`). The upstream report [hyper#3611](https://github.com/hyperium/hyper/issues/3611) ("server hangs handling a request from a client on the same host"), closed *not planned*, pins the class of bug: the hyper client connection driver must be spawned and polled concurrently, never stranded. A connection whose driver was spawned on a runtime that later stops polling it — exactly what `run_isolated_blocking`'s transient runtime did — wedges on its next reuse. `pool_max_idle_per_host(0)` mitigated it by forcing a freshly-polled driver every request, at the cost of a TLS handshake per request; the long-lived shared client on the stable runtime is the proper fix, reclaiming that cost without ever stranding a driver. No dependency bump was available to retire it: the workspace is already on the latest hyper (1.10.1) and hyper-util (0.1.20), reqwest 0.13's changes are HTTP/3-only, and the build is past reqwest 0.12.8's pooled-connection-expiry fix.

The shared-client architecture was validated against a real authenticated Drive account before the per-request/isolation machinery was removed (capture procedure and results in [`docs/tls-deadlock-capture.md`](tls-deadlock-capture.md)): sequential WebDAV uploads — back-to-back bursts and idle gaps spanning the remote idle-close window — completed with the `before-send`/`after-headers` ledger balanced and no wedge. The synthetic and real-frontend reproductions in `crates/presenter-webdav/tests/tls_topology_repro.rs` and `tls_repro_harness.rs` never reproduced the hang in any configuration; they are retained, `#[ignore]`d, as regression tooling.

The one backend-internal hazard worth recording: the refresh slow path of `GdriveBackend::access_token` locks the token `Mutex`, clones the refresh token, drops the guard, awaits the refresh, then re-locks to store the result — it must never hold the guard across the refresh await. The integration test `concurrent_refresh_through_shared_client_does_not_deadlock` fires many concurrent callers at a deliberately delayed mock OAuth2 endpoint to prove the re-acquisition never deadlocks.

One residual caveat: the original hang was never reproduced in any harness, so the captures prove the shared client works in those runs but not that a harness would catch a regression. `Connection: close` is still forced on every WebDAV response — that addresses a separate axum keep-alive flush issue, not the Drive client, and was active throughout the passing captures.
